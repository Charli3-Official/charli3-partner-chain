
//! Autogenerated weights for pallet_session_validator_management
//!
//! THIS FILE WAS AUTO-GENERATED USING THE SUBSTRATE BENCHMARK CLI VERSION 51.0.0
//! DATE: 2025-10-08, STEPS: `500`, REPEAT: `200`, LOW RANGE: `[]`, HIGH RANGE: `[]`
//! WORST CASE MAP SIZE: `1000000`
//! HOSTNAME: `Nikolaoss-MacBook-Pro.local`, CPU: `<UNKNOWN>`
//! EXECUTION: , WASM-EXECUTION: Compiled, CHAIN: None, DB CACHE: 1024

// Executed Command:
// frame-omni-bencher
// v1
// benchmark
// pallet
// --runtime
// target/release/wbuild/partner-chains-demo-runtime/partner_chains_runtime.compact.compressed.wasm
// --pallet
// pallet_session_validator_management
// --extrinsic
// *
// --steps=500
// --repeat=200
// --wasm-execution=compiled
// --output
// toolkit/committee-selection/pallet/src/weights.rs
// --template
// .maintain/frame-weight-template.hbs

#![cfg_attr(rustfmt, rustfmt_skip)]
#![allow(unused_parens)]
#![allow(unused_imports)]
#![allow(missing_docs)]

use frame_support::{traits::Get, weights::{Weight, constants::RocksDbWeight}};
use core::marker::PhantomData;

/// Weight functions needed for pallet_session_validator_management.
pub trait WeightInfo {
	fn set(v: u32, ) -> Weight;
	fn set_main_chain_scripts() -> Weight;
}

/// Weights for pallet_session_validator_management using the Substrate node and recommended hardware.
pub struct SubstrateWeight<T>(PhantomData<T>);
impl<T: frame_system::Config> WeightInfo for SubstrateWeight<T> {
	/// Storage: `SessionCommitteeManagement::CurrentCommittee` (r:1 w:0)
	/// Proof: `SessionCommitteeManagement::CurrentCommittee` (`max_values`: Some(1), `max_size`: Some(133130), added: 133625, mode: `MaxEncodedLen`)
	/// Storage: `SessionCommitteeManagement::NextCommittee` (r:1 w:1)
	/// Proof: `SessionCommitteeManagement::NextCommittee` (`max_values`: Some(1), `max_size`: Some(133130), added: 133625, mode: `MaxEncodedLen`)
	/// The range of component `v` is `[0, 1024]`.
	fn set(v: u32, ) -> Weight {
		// Proof Size summary in bytes:
		//  Measured:  `131`
		//  Estimated: `134615`
		// Minimum execution time: 10_000_000 picoseconds.
		Weight::from_parts(12_492_663, 134615)
			// Standard Error: 40
			.saturating_add(Weight::from_parts(75_113, 0).saturating_mul(v.into()))
			.saturating_add(T::DbWeight::get().reads(2_u64))
			.saturating_add(T::DbWeight::get().writes(1_u64))
	}
	/// Storage: `SessionCommitteeManagement::MainChainScriptsConfiguration` (r:0 w:1)
	/// Proof: `SessionCommitteeManagement::MainChainScriptsConfiguration` (`max_values`: Some(1), `max_size`: Some(178), added: 673, mode: `MaxEncodedLen`)
	fn set_main_chain_scripts() -> Weight {
		// Proof Size summary in bytes:
		//  Measured:  `0`
		//  Estimated: `0`
		// Minimum execution time: 2_000_000 picoseconds.
		Weight::from_parts(2_000_000, 0)
			.saturating_add(T::DbWeight::get().writes(1_u64))
	}
}

// For backwards compatibility and tests
impl WeightInfo for () {
	/// Storage: `SessionCommitteeManagement::CurrentCommittee` (r:1 w:0)
	/// Proof: `SessionCommitteeManagement::CurrentCommittee` (`max_values`: Some(1), `max_size`: Some(133130), added: 133625, mode: `MaxEncodedLen`)
	/// Storage: `SessionCommitteeManagement::NextCommittee` (r:1 w:1)
	/// Proof: `SessionCommitteeManagement::NextCommittee` (`max_values`: Some(1), `max_size`: Some(133130), added: 133625, mode: `MaxEncodedLen`)
	/// The range of component `v` is `[0, 1024]`.
	fn set(v: u32, ) -> Weight {
		// Proof Size summary in bytes:
		//  Measured:  `131`
		//  Estimated: `134615`
		// Minimum execution time: 10_000_000 picoseconds.
		Weight::from_parts(12_492_663, 134615)
			// Standard Error: 40
			.saturating_add(Weight::from_parts(75_113, 0).saturating_mul(v.into()))
			.saturating_add(RocksDbWeight::get().reads(2_u64))
			.saturating_add(RocksDbWeight::get().writes(1_u64))
	}
	/// Storage: `SessionCommitteeManagement::MainChainScriptsConfiguration` (r:0 w:1)
	/// Proof: `SessionCommitteeManagement::MainChainScriptsConfiguration` (`max_values`: Some(1), `max_size`: Some(178), added: 673, mode: `MaxEncodedLen`)
	fn set_main_chain_scripts() -> Weight {
		// Proof Size summary in bytes:
		//  Measured:  `0`
		//  Estimated: `0`
		// Minimum execution time: 2_000_000 picoseconds.
		Weight::from_parts(2_000_000, 0)
			.saturating_add(RocksDbWeight::get().writes(1_u64))
	}
}
